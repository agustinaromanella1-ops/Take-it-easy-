...
function seeded(): AppData {
  let state = reducer(emptyData(), {
    type: 'patient/add',
    payload: { name: 'Ana', email: '', phone: '', defaultFee: 500000, status: 'activo', colorIndex: 0, frequency: 'semanal', kind: 'particular', legalName: '', document: '', taxId: '', taxCondition: 'consumidor_final', memberNumber: '', insurer: '', lastRaise: null, notes: '' },
  });
...
describe('pacientes', () => {
  it('asigna un id y una fecha de alta', () => {
    const state = reducer(emptyData(), {
      type: 'patient/add',
      payload: { name: 'Ana', email: '', phone: '', defaultFee: 0, status: 'activo', colorIndex: 0, frequency: 'semanal', kind: 'particular', legalName: '', document: '', taxId: '', taxCondition: 'consumidor_final', memberNumber: '', insurer: '', lastRaise: null, notes: '' },
    });
  ...
  it('genera ids distintos para cada alta', () => {
    ...
    for (const name of ['Ana', 'Bruno', 'Carla']) {
      state = reducer(state, {
        type: 'patient/add',
        payload: { name, email: '', phone: '', defaultFee: 0, status: 'activo', colorIndex: 0, frequency: 'semanal', kind: 'particular', legalName: '', document: '', taxId: '', taxCondition: 'consumidor_final', memberNumber: '', insurer: '', lastRaise: null, notes: '' },
      });
    }
  ...
  it('borrar un paciente no toca los datos de otro', () => {
    ...
    state = reducer(state, {
      type: 'patient/add',
      payload: { name: 'Bruno', email: '', phone: '', defaultFee: 0, status: 'activo', colorIndex: 0, frequency: 'semanal', kind: 'particular', legalName: '', document: '', taxId: '', taxCondition: 'consumidor_final', memberNumber: '', insurer: '', lastRaise: null, notes: '' },
    });
...
describe('inmutabilidad', () => {
  it('no muta el estado anterior', () => {
    ...
    reducer(state, {
      type: 'patient/add',
      payload: { name: 'Nuevo', email: '', phone: '', defaultFee: 0, status: 'activo', colorIndex: 0, frequency: 'semanal', kind: 'particular', legalName: '', document: '', taxId: '', taxCondition: 'consumidor_final', memberNumber: '', insurer: '', lastRaise: null, notes: '' },
    });
